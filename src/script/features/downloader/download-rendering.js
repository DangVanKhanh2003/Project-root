 ...
 */
export function renderDownloadOptions(state) {
 ...
 */
function renderVideoInfoSmart(meta) {
    const existingThumbnail = document.querySelector('#videoThumbnail');
    const existingTitle = document.querySelector('#videoTitle');
    ...
    if (existingThumbnail && existingTitle && meta && meta.thumbnail) {
        ...
        if (currentSrc === newSrc) {
            ...
            setTimeout(() => {
                const titleElement = document.querySelector('#videoTitle');
 ...
 */
export function renderVideoInfo(meta) {
    if (!meta) {
 ...
 */
export function updateVideoTitle(meta) {
 ...
 */
export function renderTabNavigation(activeTab, hasVideo, hasAudio) {
 ...
 */
export function renderFormatPanels(videoFormats, audioFormats, activeTab, downloadTasks) {
 ...
 */
export function renderFormatPanel(category, formats, isActive, downloadTasks) {
 ...
 */
export function renderFormatItem(format, downloadTasks) {
 ...
 */
function renderConversionButton(format, downloadTasks) {
    ...
    const taskState = downloadTasks[formatId] || { status: 'idle' };
 ...
 */
function renderDirectDownloadButton(format) {
    const formatId = format.id;
 ...
 */
function renderCenterColumn(format, conversionTask) {
 ...
 */
function getButtonStateForTask(taskState, conversionTask) {
 ...
 */
function renderButtonIcon(buttonClass) {
 ...
 */
export function attachDownloadListeners(container) {
    if (!container) {
        return;
    }
    ...
    container.addEventListener('click', (event) => {
        ...
        const downloadBtn = event.target.closest('.quality-item');
        if (downloadBtn) {
 ...
 * @param {HTMLElement} container - Download options container
 */
function setupImageLoader(container) {
    const img = container.querySelector('#videoThumbnail');
    const skeleton = container.querySelector('.thumbnail-skeleton');
...
async function handleDownloadClick(event) {
    const button = event.target.closest('.btn-convert');
    if (!button) return;
 ...
 */
function extractFormatDataFromState(formatId) {
    try {
        const state = getState();
