type FeedbackWidgetTranslations = {
  bubbleText: string;
  title: string;
  emailLabel: string;
  ideaLabel: string;
...
type FeedbackSummary = {
  ...
  itemsCount: number;
  hasVideoDetail: boolean;
  hasGalleryDetail: boolean;
  conversionTaskCount: number;
  downloadTaskCount: number;
};

export type FeedbackWidgetPayload = {
  title: string;
  description: string;
  page: string;
  email: string;
...
export type InitFeedbackWidgetOptions = {
  sendFeedbackWidget: (payload: FeedbackWidgetPayload) => Promise<FeedbackWidgetResult>;
  getDownloaderState?: () => unknown;
  getInputValue?: () => string;
  translations?: Partial<FeedbackWidgetTranslations>;
  cssId?: string;
  wrapperId?: string;
  titlePrefix?: string;
};

const DEFAULT_TRANSLATIONS: FeedbackWidgetTranslations = {
...
let elements: FeedbackWidgetElements = {};

function defaultGetInputValue(): string {
  const selectors = ['#urlsInput', '#playlistUrl', '#videoUrl', '#youtubeUrl', '#urlInput'];
...
function buildSanitizedContext(getDownloaderState?: () => unknown): any {
  if (!getDownloaderState) return null;

  let cloned: any = null;
  try {
    cloned = JSON.parse(JSON.stringify(getDownloaderState()));
  } catch {
    cloned = null;
  }
...
function buildFeedbackSummary(context: any): FeedbackSummary {
  return {
...
function injectStyles(cssId: string): void {
  if (document.getElementById(cssId)) return;

  const style = document.createElement('style');
  style.id = cssId;
  style.innerHTML = `
    .fb-wrap { position: fixed; right: 10px; bottom: 16px; display: flex; flex-direction: column; align-items: flex-end; gap: 12px; z-index: 9999; font-family: system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; box-sizing: border-box; }
    .fb-wrap * { box-sizing: border-box; }
    .fb-bubble { height: 46px; width: fit-content; display: inline-flex; align-items: center; gap: 8px; padding: 8px 12px; border-radius: 999px; border: 1px solid #e2e8f0; background: #fff; color: #334155; cursor: pointer; box-shadow: 0 18px 45px rgba(2, 6, 23, 0.12), 0 2px 6px rgba(2, 6, 23, 0.10); transition: transform .2s ease, box-shadow .2s ease; user-select: none; font-family: inherit; }
    .fb-bubble:hover { box-shadow: 0 24px 70px rgba(2, 6, 23, 0.16), 0 3px 8px rgba(2, 6, 23, 0.12); background: #f8fafc; }
    ...
    @media (max-width: 768px) { .fb-bubble { width: 46px; min-width: 46px; justify-content: center; padding: 0; } .fb-bubble svg { width: 22px; height: 22px; } .fb-bubble-text { display: none; } }
    .fb-panel { width: 300px; max-width: calc(100vw - 48px); background: #fff; border: 1px solid #f1f5f9; border-radius: 18px; box-shadow: 0 24px 70px rgba(2, 6, 23, 0.16); overflow: hidden; transform-origin: bottom right; animation: fbIn .28s cubic-bezier(0.16, 1, 0.3, 1) both; }
    .fb-hidden { display: none !important; }
    @keyframes fbIn { from { opacity: 0; transform: translateY(10px) scale(.96); } to { opacity: 1; transform: translateY(0) scale(1); } }
    @keyframes fbSpin { to { transform: rotate(360deg); } }
    .fb-head { display: flex; align-items: center; justify-content: space-between; padding: 12px 14px; background: #f8fafc; border-bottom: 1px solid #f1f5f9; }
    .fb-title { display: flex; align-items: center; gap: 8px; margin: 0; font-size: 13px; font-weight: 700; color: #1f2937; }
    .fb-close { border: 0; background: transparent; color: #94a3b8; cursor: pointer; padding: 6px; border-radius: 999px; transition: background .15s ease, color .15s ease, transform .12s ease; display: flex; align-items: center; justify-content: center; }
    .fb-close:hover { color: #475569; background: #e2e8f0; }
    .fb-close:active { transform: scale(.96); }
    .fb-close svg { width: 16px; height: 16px; }
    .fb-body { padding: 14px; display: flex; flex-direction: column; gap: 12px; }
    .fb-field label { display: block; font-size: 11px; font-weight: 800; letter-spacing: .08em; text-transform: uppercase; color: #64748b; margin: 0 0 6px 4px; }
    .fb-field label .req { color: #f87171; }
    .fb-control { position: relative; }
    .fb-control .ic { position: absolute; left: 12px; width: 16px; height: 16px; color: #94a3b8; pointer-events: none; transition: color .15s ease; display: flex; align-items: center; justify-content: center; top: 50%; transform: translateY(-50%); }
    .fb-input, .fb-textarea { width: 100%; font-size: 16px; padding: 10px 12px 10px 36px; border-radius: 14px; border: 1px solid #e2e8f0; background: #f8fafc; color: #0f172a; outline: none; transition: border-color .15s ease, box-shadow .15s ease, background .15s ease; font-family: inherit; }
    .fb-textarea { resize: none; padding-left: 12px; }
    .fb-input::placeholder, .fb-textarea::placeholder { color: #94a3b8; }
    .fb-input:focus, .fb-textarea:focus { background: #fff; border-color: #6366f1; box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.10); }
    .fb-submit { width: 100%; border: 0; border-radius: 14px; padding: 11px 12px; background: #0f172a; color: #fff; font-size: 14px; font-weight: 600; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: transform .12s ease, background .15s ease, opacity .15s ease; box-shadow: 0 12px 24px rgba(2, 6, 23, 0.10); margin-top: 4px; font-family: inherit; }
    ...
    .fb-success { padding: 28px 18px; min-height: 230px; display: flex; flex-direction: column; align-items: center; justify-content: center; text-align: center; gap: 8px; animation: fbIn .28s cubic-bezier(0.16, 1, 0.3, 1) both; }
    .fb-badge { width: 56px; height: 56px; border-radius: 999px; background: #ecfdf5; color: #22c55e; display: flex; align-items: center; justify-content: center; margin-bottom: 2px; }
...
export function initFeedbackWidget(options: InitFeedbackWidgetOptions): void {
  if (globalState.initialized) return;

  const cssId = options.cssId || 'fb-widget-css';
  const wrapperId = options.wrapperId || 'fb-widget-wrap';
  const titlePrefix = options.titlePrefix || 'Feedback Widget';
  const getInputValue = options.getInputValue || defaultGetInputValue;
  const text: FeedbackWidgetTranslations = {
  ...
  container.innerHTML = `
    <div id="fb-panel" class="fb-panel fb-hidden">
      <div id="fb-view-form">
        <div class="fb-head">
          <h3 class="fb-title"><span style="color:#eab308;display:flex;">${icons.lightbulb}</span>${text.title}</h3>
        ...
        <form id="fb-form" class="fb-body">
          ...
          <div class="fb-field">
            <label>${text.ideaLabel} <span class="req">*</span></label>
            <div class="fb-control">
              <textarea id="fb-idea" class="fb-textarea" rows="3" placeholder="${text.ideaPlaceholder}" required></textarea>
            </div>
          </div>
          <button id="fb-submit" class="fb-submit" type="submit">
            <span id="fb-btn-text">${text.submitBtn}</span>
    ...
    <button id="fb-bubble" class="fb-bubble" type="button">
      ...
      <span class="fb-bubble-text">${text.bubbleText}</span>
    </button>
  `;
  ...
  const resetForm = () => {
    ...
    elements.btnText.textContent = text.submitBtn;
  ...
  const handleSubmit = async (event: Event) => {
    ...
    elements.btnText.textContent = text.sending;
    elements.btnIcon.classList.add('fb-hidden');
    elements.btnLoader.classList.remove('fb-hidden');

    const context = buildSanitizedContext(options.getDownloaderState);
    ...
    try {
      const result = await options.sendFeedbackWidget({
  ...
  elements.bubble?.addEventListener('click', (e) => {
    e.stopPropagation();
    if (globalState.isOpen) closePanel();
    else openPanel();
  });
