    ...
    if (autoDownload) {
      ...
      if (is4K) {
        ...
        if (!limitResult.allowed) {
          showPaywall('download_4k', { secondaryLabel: 'Continue without 4K', onSecondaryClick: () => { setVideoQuality('720p'); const badge = document.querySelector('.badge-main-quality'); if (badge) badge.textContent = '720p'; const sel = document.getElementById('quality-select-mp4') as HTMLSelectElement | null; if (sel) sel.value = 'mp4-720'; setLoading(true); setSubmitting(true); handleExtractMedia(url); } });
          setSubmitting(false);
          setLoading(false);
          return;
        }
      ...
      if (is320kbps) {
        ...
        if (!limitResult.allowed) {
          showPaywall('download_320kbps', { secondaryLabel: 'Continue without 320kbps', onSecondaryClick: () => { setAudioBitrate('128'); const badge = document.querySelector('.badge-main-quality'); if (badge) badge.textContent = '128kbps'; const sel = document.getElementById('quality-select-mp3') as HTMLSelectElement | null; if (sel) sel.value = 'mp3-128'; setLoading(true); setSubmitting(true); handleExtractMedia(url); } });
          setSubmitting(false);
          setLoading(false);
          return;
        }
