 ...
 *
 * Conversion flow with priority routing:
 * - External Extract API (cc.ytconvert.org) — direct download, no polling
 * - V3 API (hub.ytconvert.org) — create job -> poll status -> download URL
 *
 * Routing: country-based (IN/ID -> external-first, others -> v3-first)
 * Silent fallback when primary API fails.
 */
