  ...
  if (existingWrapper) {
    ...
    if (existingPreviewCard) {
      ...
      if (existingImg && existingImg.src === thumbnail) {
        ...
        const audioTrackIconEl = existingPreviewCard.querySelector('.audio-track-icon');
        if (audioTrackValueEl) audioTrackValueEl.textContent = audioTrackLabel;
        if (audioTrackIconEl) audioTrackIconEl.innerHTML = audioTrackIcon;
