...
import { getAddedCount, hasPendingUploads } from '../../logic/multiple-download/mobile-save-zip-manager';
        ...
        <div class="group-header">
            ...
            <div class="group-header-bottom-row">
                ...
                <div class="group-actions">
                    ...
                    <span class="btn-text">Download ZIP (0)</span></button>
...
export function updateGroupCount(groupEl: HTMLElement, isLocked: boolean = false): void {
    ...
    if (convertAllBtn && zipBtn) {
        if (activeTab === 'convert') {
            // Skip updating Convert button if it's in loading state
            if (!convertAllBtn.classList.contains('is-loading')) {
                ...
                if (isConvertDisabled) {
                    ...
                    // Hide checkbox hand guide when button becomes active
                    hideCheckboxHandGuide(groupEl);
                }
            }
            // Don't hide ZIP button if it's in loading state (polling in progress)
            ...
            // Skip updating ZIP button if it's in loading state
            if (!zipBtn.classList.contains('is-loading')) {
...
export const CONVERT_BUTTON_GUIDE_SHOWN_KEY = STORAGE_KEYS.PLAYLIST_CONVERT_GUIDE_SHOWN;
...
/**
 * Toggle .is-disabled, aria-disabled and data-tooltip on a button.
 * Does NOT use HTML disabled attribute so that hover/click events still fire.
 */
...
/**
 * Show hand-click GIF pointing at the group checkbox
 * when user clicks a disabled group action button.
 */
