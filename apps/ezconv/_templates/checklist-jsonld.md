# JSON-LD Checklist cho Ezconv

## 1. Cấu trúc cơ bản

- [x] Sử dụng `@graph` array để chứa nhiều entities
- [x] Mỗi entity phải có `@type` và `@id`
- [x] `@id` format: `{canonicalUrl}#{fragment}` (vd: `https://ezconv.pro/#organization`)
...
|--------|---------------|-------|
| Organization | "Ezconv" (fixed) | Brand name |
| WebSite | "Ezconv" (fixed) | Site name |
| WebPage | `hero.title` (H1) | Tên trang/tool cụ thể |
| SoftwareApplication | `hero.title` (H1) | Tên tool cụ thể |

## Ezconv Data Structure

**Lưu ý**: Ezconv sử dụng cấu trúc data khác với y2matepro:
