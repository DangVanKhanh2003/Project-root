 ...
 *
 * Dynamically imported from multi-mode-main.ts only when ?license= param exists.
 */

import { supporterService } from '../api';
import { saveLicenseFromApi, clearLicenseToken, computeDaysRemaining } from './license-token';
import { initSupporterUi } from './shared/init/common-init';
...
function injectStyles(): void {
    const styleId = 'ezconv-auto-license-checker-styles';
    ...
    style.textContent = `
        ...
        [data-theme="dark"] .alc-status-loading { background: rgba(30, 64, 175, 0.2); color: #93c5fd; }
        [data-theme="dark"] .alc-status-success { background: rgba(6, 95, 70, 0.2); color: #6ee7b7; }
        [data-theme="dark"] .alc-status-error { background: rgba(153, 27, 27, 0.2); color: #fca5a5; }
...
export async function autoCheckLicense(licenseKey: string): Promise<void> {
    ...
    try {
        ...
        if (!isLicenseInactive(result)) {
            // ---- SUCCESS ----
            saveLicenseFromApi(licenseKey, result);
            ...
            if (result.planType !== 'lifetime' && result.expiresAt) {
                const days = computeDaysRemaining(result.expiresAt);
        ...
        subtitleEl.textContent = 'There was a network problem while verifying your key. Please check your connection and try again.';
        closeBtn.classList.add('visible');
    }
}
