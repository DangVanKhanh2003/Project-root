...
import { mapToV3DownloadRequest, type ExtractV2Options } from '@downloader/core';
...
function buildExtractV2Options(settings: VideoItemSettings): ExtractV2Options {
    ...
    const AUDIO_FORMATS_SET = new Set(['ogg', 'wav', 'opus', 'm4a', 'flac']);
    const isFormatOverride = isAudio && AUDIO_FORMATS_SET.has(settings.audioBitrate || '');

    return {
        downloadMode: isAudio ? 'audio' : 'video',
        videoQuality: isAudio ? undefined : (settings.quality?.replace('p', '') || '720'),
