 ...
 * UI stays silent during fallback — no visual change.
 */
...
import { getCachedCountry } from '../../feature-access';
 ...
 * Pure synchronous function — no UI side effects.
 *
 * Rules:
 * 1. Supporters -> always v3-first
 * 2. Format not mp3/mp4 -> v3-first (external API only supports mp3/mp4)
 * 3. Country IN/ID -> external-first
 * 4. No cached country -> v3-first (cache will warm for next time via initAllowedFeatures)
 * 5. Default -> v3-first
 */
export function resolveExtractStrategy(options: ExtractV2Options): ExtractStrategy {
  // Supporters always use V3 first
  if (hasValidLicense()) return EXTRACT_STRATEGY.V3_FIRST;
  ...
  // Country-based routing (sync — read from in-memory cache only)
  const cachedCountry = getCachedCountry();

  if (cachedCountry === null) {
    // No cache -> default to V3 first
    // initAllowedFeatures() on page load will warm cache for next request
...
// -- External Extract Caller (shared by single & multiple) --
