...
export class MultipleDownloadRenderer {
    ...
    private updateBatchHeader() {
        ...
        const batchBtnDisabled = completedSelectedCount === 0 || this.isGlobalDownloadLocked;
        ...
        headerEl.innerHTML = `
            ...
            <div class="multiple-download-group-actions">
                ...
                ${!isIOS() ? `<button type="button" class="group-download-btn btn-success${batchBtnDisabled ? ' is-disabled' : ''}" id="multiDownloadActionBtn" data-action="download-zip-batch" ${batchBtnDisabled ? 'aria-disabled="true" data-tooltip="Select items to download"' : 'aria-disabled="false"'}>
                ...
                case 'download-group':
                    if (groupId) {
                        if (actionBtn.classList.contains('is-disabled')) {
                            const groupElForHandGuide = this.groupListContainer?.querySelector(`[data-group-id="${groupId}"].playlist-group`) as HTMLElement;
