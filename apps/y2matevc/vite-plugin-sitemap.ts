 ...
 * Customized for y2matevc project
 */
export function sitemapPlugin(config: SitemapConfig = {}): Plugin {
  const {
    baseUrl = 'https://y2mate.vc', // Default base URL for y2matevc
    changefreq = 'weekly',
    defaultPriority = 0.8,
    homePriority = 1.0
  } = config;
...
const EXCLUDE_PATTERNS = [
  ...
  /^yandex_[a-z0-9]+\.html$/i // Yandex verification files
];
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      if (shouldExclude(entry.name)) {
        console.log(`⏭️  [sitemap] Skipping: ${entry.name}`);
        continue;
      }
