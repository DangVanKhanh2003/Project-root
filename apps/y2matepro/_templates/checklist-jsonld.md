# JSON-LD Checklist cho Web Apps

## 1. Cấu trúc cơ bản

- [ ] Sử dụng `@graph` array để chứa nhiều entities
- [ ] Mỗi entity phải có `@type` và `@id`
- [ ] `@id` format: `{canonicalUrl}#{fragment}` (vd: `https://example.com/#organization`)
...
- [ ] **Organization**: Định nghĩa 1 lần duy nhất
  - `name`, `url`, `logo`, `contactPoint`
- [ ] **WebSite**: Định nghĩa 1 lần duy nhất
  - `publisher` → `#organization`
- [ ] **WebPage**: Có `isPartOf` → `#website`, `about` → `#application`
- [ ] **SoftwareApplication**: `name` = H1 (hero.title)

## 4. Trang chủ ngôn ngữ khác

- [ ] **WebPage**: `isPartOf` → `#website`, `about` → `#application`
- [ ] **SoftwareApplication**: `name` = H1 (hero.title theo ngôn ngữ)
- [ ] KHÔNG định nghĩa lại Organization/WebSite
