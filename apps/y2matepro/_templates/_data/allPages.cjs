...
module.exports = function() {
  ...
  // Define all supported languages (must match site.languages in .eleventy.cjs)
  const languages = [
  ...
  if (!fs.existsSync(pagesDir)) {
    console.warn('[allPages.js] Pages directory not found:', pagesDir);
    return allPages;
  }
  ...
  pageConfigs.forEach(pageConfig => {
    ...
    if (!existingPageFolders.includes(pageConfig.pageKey)) {
      console.warn(`[allPages.js] Skipping ${pageConfig.pageKey} - no data folder found`);
      return;
    }
    ...
    languages.forEach(langInfo => {
      ...
      if (!availableLangs.includes(langInfo.code)) {
        console.warn(`[allPages.js] Skipping ${pageConfig.pageKey}/${langInfo.code} - no data file`);
        return;
      }
      ...
      if (langInfo.isDefault) {
        // English (default): /index.html or /youtube-to-mp4.html
        permalink = pageConfig.slug === ''
          ? '/index.html'
          : `/${pageConfig.slug}.html`;
      } else {
        // Other languages: /vi/index.html or /vi/youtube-to-mp4.html
        permalink = pageConfig.slug === ''
  ...
  console.log(`[allPages.js] Generated ${allPages.length} page variants`);
  return allPages;
};
