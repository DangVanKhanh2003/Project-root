# Prompt: Clone onedownloader.net → tube1s.com
...
Nhiệm vụ: tạo site mới `tube1s.com` bằng cách copy `apps/onedownloader.net` rồi rebrand.
...
```bash
cp -r apps/onedownloader.net apps/tube1s.com
```
...
## Rebrand: OneDownloader → Tube1s

Thay thế theo thứ tự (specific trước, generic sau):
| Tìm | Thay |
|-----|------|
| `onedownloader.net` | `tube1s.com` |
| `OneDownloader` | `Tube1s` |
| `onedownloader` | `tube1s` |
| `ONEDOWNLOADER` | `TUBE1S` |
...
Config:
- `package.json` → `"name": "tube1s.com"`
- `.eleventy.cjs` → `url: 'https://tube1s.com'`
- `public/robots.txt` → `Sitemap: https://tube1s.com/sitemap.xml`
...
Source code:
- `src/utils/storage-keys.ts` → `PREFIX = 'tube1s'`
- `src/api/index.ts` → `createNamespacedKey('tube1s', 'downloader')`
- `src/features/download-limit.ts` → `LICENSE_KEY_STORAGE_KEY = 'tube1s:license_key'` + comment
- `src/features/license/license-token.ts` → `INTEGRITY_SALT = 'tube1s_lk_2026'` + comments
...
- `public/api-logger.worker.js` → `dbName: 'tube1s_ApiLogsDB'`
...
Update `rebrand.cjs` với patterns mới cho tube1s.
...
1. `workflow_dispatch.inputs.specific_app.options` — thêm `'tube1s.com'`
2. `dorny/paths-filter filters` — thêm `tube1s.com: - 'apps/tube1s.com/**'`
3. `ALL_APPS` JSON array — thêm `"tube1s.com"`
4. Detect changes block — thêm if block cho `steps.filter.outputs['tube1s.com']`
5. Deploy job — thêm `deploy-tube1s-com` job
6. Summary job — thêm vào `needs` + table row

**`.github/workflows/deploy-manual.yml`** — thêm 2 chỗ:
1. `workflow_dispatch.inputs.app.options` — thêm `'tube1s.com'`
2. `case` block — thêm config cho `tube1s.com`

Domain variable: `vars.TUBE1S_DOMAIN` (fallback `tube1s.com`)

## Lưu ý quan trọng

1. **KHÔNG sửa bất kỳ file nào ngoài `apps/tube1s.com/` và `.github/workflows/`** — các site cũ không được ảnh hưởng
...
5. **SEO content** trong `_templates/_data/pages/index/en.json` — nên viết lại phù hợp với brand Tube1s, không copy y nguyên
