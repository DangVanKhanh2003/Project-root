# Prompt: Clone onedownloader.net → convert1s.com
...
Nhiệm vụ: tạo site mới `convert1s.com` bằng cách copy `apps/onedownloader.net` rồi rebrand.
...
```bash
cp -r apps/onedownloader.net apps/convert1s.com
```
...
## Rebrand: OneDownloader → Convert1s

Thay thế theo thứ tự (specific trước, generic sau):
| Tìm | Thay |
|-----|------|
| `onedownloader.net` | `convert1s.com` |
| `OneDownloader` | `Convert1s` |
| `onedownloader` | `convert1s` |
| `ONEDOWNLOADER` | `CONVERT1S` |
...
Config:
- `package.json` → `"name": "convert1s.com"`
- `.eleventy.cjs` → `url: 'https://convert1s.com'`
- `public/robots.txt` → `Sitemap: https://convert1s.com/sitemap.xml`
...
Source code:
- `src/utils/storage-keys.ts` → `PREFIX = 'convert1s'`
- `src/api/index.ts` → `createNamespacedKey('convert1s', 'downloader')`
- `src/features/download-limit.ts` → `LICENSE_KEY_STORAGE_KEY = 'convert1s:license_key'` + comment
- `src/features/license/license-token.ts` → `INTEGRITY_SALT = 'convert1s_lk_2026'` + comments
...
- `public/api-logger.worker.js` → `dbName: 'convert1s_ApiLogsDB'`
...
Update `rebrand.cjs` với patterns mới cho convert1s.
...
1. `workflow_dispatch.inputs.specific_app.options` — thêm `'convert1s.com'`
2. `dorny/paths-filter filters` — thêm `convert1s.com: - 'apps/convert1s.com/**'`
3. `ALL_APPS` JSON array — thêm `"convert1s.com"`
4. Detect changes block — thêm if block cho `steps.filter.outputs['convert1s.com']`
5. Deploy job — thêm `deploy-convert1s-com` job
6. Summary job — thêm vào `needs` + table row

**`.github/workflows/deploy-manual.yml`** — thêm 2 chỗ:
1. `workflow_dispatch.inputs.app.options` — thêm `'convert1s.com'`
2. `case` block — thêm config cho `convert1s.com`

Domain variable: `vars.CONVERT1S_DOMAIN` (fallback `convert1s.com`)

## Lưu ý quan trọng

1. **KHÔNG sửa bất kỳ file nào ngoài `apps/convert1s.com/` và `.github/workflows/`** — các site cũ không được ảnh hưởng
...
5. **SEO content** trong `_templates/_data/pages/index/en.json` — nên viết lại phù hợp với brand Convert1s, không copy y nguyên
